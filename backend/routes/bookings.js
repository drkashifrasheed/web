...
const { generateOTP, sendBookingCancellationEmail } = require('../utils/emailService');
...
router.post('/:id/request-video-call', protect, adminOnly, async (req, res, next) => {
  try {
    ...
    await notifyUser(booking.patient, {
      ...
      message: 'Dr. Mahar Kashif Rasheed invited you to a video call. Open chat or profile to accept.',
      bookingId: booking._id,
      actionUrl: `/video-call/${booking._id}`,
      priority: 'high'
    });
    ...
    await Message.create({
      ...
      bookingId: booking._id,
      content:
        '📹 Video consultation request: Please accept to join a secure video call with the doctor.',
