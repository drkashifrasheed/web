    ...
    <div className="min-h-[100dvh] bg-gray-50 flex flex-col lg:flex-row pb-[4.5rem] lg:pb-0">
      ...
      {selectedBooking && (
        <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-[60] p-0 sm:p-4 safe-top safe-bottom">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl max-w-lg w-full max-h-[92dvh] sm:max-h-[90vh] overflow-y-auto p-4 sm:p-6">
            <h3 className="text-lg font-bold mb-4">Manage booking</h3>
            <div className="text-sm space-y-2 mb-4 p-4 bg-gray-50 rounded-xl">
              <p>
              ...
              {selectedBooking.paymentProofUrl && (
                <a
                  href={`${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '') || 'http://localhost:5000'}${selectedBooking.paymentProofUrl}`}
                  target="_blank"
                  rel="noreferrer"
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    await bookingApi.requestVideoCall(selectedBooking._id)
                    toast.success('Video call request sent to patient')
                    await loadBookings()
                  } catch {
              ...
              {selectedBooking.status === 'accepted' && (
                <Link
                  href={`/video-call/${selectedBooking._id}`}
            ...
            <div className="space-y-3">
              ...
              {actionForm.status === 'accepted' && (
                <>
                  ...
                  <input
                    placeholder="Meeting link (video calls)"
